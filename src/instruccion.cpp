...
instruccion::instruccion(short op, short tipo, int val)
    : opcode(op), especial(tipo), value(val) {}
...
instruccionReg::instruccionReg(short op, short tipo, int value)
    : instruccion(op, tipo, value) {}

instruccionCinta::instruccionCinta(short op, short tipo, int value)
    : instruccion(op, tipo, value) {}

instruccionAlu::instruccionAlu(short op, short tipo, int value)
    : instruccion(op, tipo, value) {}

instruccionSalto::instruccionSalto(short op, short etiquetaSalida)
    : instruccion(op, -1, etiquetaSalida) {}
...
int instruccionAlu::valor(RAM* r) {
  if (especial == NADA)  // NADA 1     → retorno el contenido de R1
    ...
    return value;
           ...
           ESTRELLA)  // IGUAL* 1   → retorna el contenido de lo apuntado por R1
...
bool instruccionCinta::ejecutar(RAM* r) {
  switch (opcode) {
    case READ:  // leer la cinta de entrada. _No se puede leer el acumulador_
      ...
      else  // READ *1  → lo que lea la cabeza de lectura a la dir cotnenida en
            // R1
        r->setReg(r->getReg(value), r->read());
      break;
    case WRITE:              // escribir la cinta de salida
      if (especial == NADA)  // WRITE 1   → lo que está en R1 se escribe en
                             // dónde esté la cabeza de escritura
        r->write(r->getReg(value));
      else if (especial == IGUAL)  // WRITE = 1 → escribir 1 en dónde esté la
                                   // cabeza de escritura
        r->write(value);
      else  // WRITE *1  → lo que esté en lo apuntado por R1 se escribe en la
            // cinta de escritura
...
bool instruccionHalt::ejecutar(RAM* r) { r->setHalt(); }
...
string instruccion::decodificacion() {
  if (especial == NADA)
    ...
    return "el registro al que apunta R";
}
...
string instruccionAlu::decodificacionSemantica() {
  switch (opcode) {
    case ADD:
      return "Leer Acumulador, sumarle" + queCosa() + decodificacion() +
             to_string(value) + " y guardar en el Acumulador";
      break;
    case SUB:
      return "Leer Acumulador, restarle" + queCosa() + decodificacion() +
             to_string(value) + " y guardar en el Acumulador";
      break;
    case MUL:
      return "Leer Acumulador, multiplicarlo por" + queCosa() +
             decodificacion() + to_string(value) +
             " y guardar en el Acumulador";
      break;
    case DIV:
      return "Leer Acumulador, dividirlo por" + queCosa() + decodificacion() +
             to_string(value) + " y guardar en el Acumulador";
      break;
  }
}
...
string instruccionReg::decodificacionSemantica() {
  switch (opcode) {
    case LOAD:
      return "Guardar en el Acumulador " + queCosa() + decodificacion() +
             to_string(value);
      break;
    case STORE:
      return "Guardar en " + decodificacion() + to_string(value) +
...
string instruccionCinta::decodificacionSemantica() {
  switch (opcode) {
    case READ:
      if (especial == NADA)
        return "Guardar en el registro " + to_string(value) +
               " lo que lea la cabeza de lectura";
      else
        return "Guardar en " + decodificacion() + to_string(value) +
               " lo que lea la cabeza de lectura";
      break;
    case WRITE:
      return queCosa() + decodificacion() + to_string(value) +
             " se envía a la cabeza de escritura";
      break;
  }
}
...
string instruccionSalto::decodificacionSemantica() {
  switch (opcode) {
    case JUMP:
      ...
      break;
    case JGTZ:
      return "Salto si Acumulador > 0 a la dirección virtual " +
             to_string(value);
      break;
    case JZERO:
      return "Salto si Acumulador = 0 a la dirección virtual " +
